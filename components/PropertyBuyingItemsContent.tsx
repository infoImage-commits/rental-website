...
import {
  ...
  useCreatePropertyBuyingCategoryItem,
...
function ItemFormPanel({
  ...
  item: PropertyBuyingCategoryItem | null;
  isOpen: boolean;
  onClose: () => void;
  onSaved: () => void;
}) {
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-white shadow-2xl">
        ...
        <form onSubmit={handleSave} className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
          ...
          <div>
            ...
            <select
              required
              value={propertyBuyingCategoryId}
              onChange={(e) => setPropertyBuyingCategoryId(e.target.value)}
...
// ── Main content ──────────────────────────────────────────────────────────────
export default function PropertyBuyingItemsContent() {
  const [isPanelOpen, setIsPanelOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<PropertyBuyingCategoryItem | null>(null);
  const [itemToDelete, setItemToDelete] = useState<PropertyBuyingCategoryItem | null>(null);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<string>("all");

  const { data: items = [], isLoading: itemsLoading, isError: itemsError } = usePropertyBuyingCategoryItems();
  const { data: categories = [], isLoading: categoriesLoading } = usePropertyBuyingCategories();
  const { mutate: deleteItem } = useDeletePropertyBuyingCategoryItem();
  const { mutate: updateStatus } = useUpdatePropertyBuyingCategoryItemStatus();
  ...
  function openEditPanel(item: PropertyBuyingCategoryItem) {
    setEditingItem(item);
    setIsPanelOpen(true);
  }
  ...
  const filteredItems = selectedCategory === "all" ? items : items.filter(item => item.propertyBuyingCategoryId === selectedCategory);
          ...
          <table className="w-full text-left text-[14px]">
            ...
            <tbody className="divide-y divide-[#f0f4f2]">
              {filteredItems.map((item) => {
                const category = categories.find(c => c.id === item.propertyBuyingCategoryId);
